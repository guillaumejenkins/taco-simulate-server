...
var telemetry;
var pluginId = 'cordova-plugin-geolocation';
var panelId = 'geolocation';
...
module.exports = function (messages) {
    ...
    function sendUITelemetry(controlData) {
        if (telemetry) {
            var props = { pluginId: pluginId, panel: panelId };

            if (typeof controlData === "string") {
                props.controlId = controlData;
            } else {
                props.controlId = controlData.id;
                props.value = controlData.value;
            }
    ...
    function mapEventTelemetryHandler() {
        ...
        setTimeout(function () {
            ...
            if (pendingMouseEvents === 0) {
                sendUITelemetry('geo-map-container');
            }
        }, mouseEventHoldDelay);
    }

    function registerTelemetryEvents() {
        document.getElementById('geo-latitude').onchange = sendUITelemetry.bind(this, 'geo-latitude');
        document.getElementById('geo-longitude').onchange = sendUITelemetry.bind(this, 'geo-longitude');
        document.getElementById('geo-altitude').onchange = sendUITelemetry.bind(this, 'geo-altitude');
        document.getElementById('geo-accuracy').onchange = sendUITelemetry.bind(this, 'geo-accuracy');
        document.getElementById('geo-altitude-accuracy').onchange = sendUITelemetry.bind(this, 'geo-altitude-accuracy');
        document.getElementById('geo-heading').onchange = sendUITelemetry.bind(this, 'geo-heading');
        document.getElementById('geo-speed').onchange = sendUITelemetry.bind(this, 'geo-speed');
        document.getElementById('geo-delay').onchange = sendUITelemetry.bind(this, 'geo-delay');
        ...
        geoTimeoutCheckbox.onclick = function () {
            if (geoTimeoutCheckbox.checked !== previousTimeoutState) {
                ...
                sendUITelemetry('geo-timeout');
            }
        };
        document.getElementById('geo-gpxfile-button').onclick = sendUITelemetry.bind(this, 'geo-gpxfile-button');
        document.getElementById('geo-gpx-go').onclick = function () {
            ...
            sendUITelemetry({ id: 'geo-gpx-go', value: option.value });
        }
        document.getElementById('geo-map-container').onwheel = mapEventTelemetryHandler;
        document.getElementById('geo-map-zoom-decrease').onclick = sendUITelemetry.bind(this, 'geo-map-zoom-decrease');
        document.getElementById('geo-map-zoom-increase').onclick = sendUITelemetry.bind(this, 'geo-map-zoom-increase');
    }
