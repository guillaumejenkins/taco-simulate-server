...
function handleWatcherEvent(root, fileRelativePath) {
    if (!fileRelativePath) {
        // fs.watch() doesn't always set the fileRelativePath argument properly. If that happens, error out.
        throw new Error('Could not reload the modified file: fs.watch() didn\'t specify which file was changed');
    }

    // fs.watch() will often send events more than once for the same modification, especially on Windows. A workaround is to block events generated by the same file for a shlort duration.
    if (this.ignoreEvents[fileRelativePath]) {
        return;
    }

    this.ignoreEvents[fileRelativePath] = true;
    setTimeout(function () {
        this.ignoreEvents[fileRelativePath] = false;
    ...
    // At this point, we either prepare the whole platform or copy only the modified file, based on the --noprepare command-line parameter.
    var preparePromise;

    if (config.prepareOnChange) {
    ...
    preparePromise.then(function () {
        if (fileIsOfType(fileRelativePath, '.css')) {
